        ...
        <nav class="navbar navbar-expand-md avbar-dark bg-dark">
            <div class="container-fluid">
                ...
                <form class="container-fluid justify-content-start">
                    <button class="btn btn-sm btn-outline-secondary" onclick="location.href='./terms'">利用規約</button>
                    <button class="btn btn-sm btn-outline-secondary" onclick="location.href='./privacy-policy'">プライバシーポリシー</button>
                    <button class="btn btn-sm btn-outline-secondary" onclick="window.open('https:\/\/discord.gg/7xHNfqYgxb')">サポートサーバー</button>
                </form>
                <form class="form-inline">
                    <?php if(!isset($_SESSION["user"])){ ?>
                        <button class="btn btn-outline-success my-2 my-sm-0" onclick="location.href='<?= url($client_id,$redirect_url,$scopes) ?>'">ログイン</button>
                    <?php }else{ ?>
                        <button class="btn btn-outline-danger my-2 my-sm-0" onclick="location.href='./includes/logout'">ログアウト</button>
                    <?php } ?>
                </form>
            </div>
        </nav>
